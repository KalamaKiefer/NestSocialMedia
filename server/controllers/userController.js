...
exports.resizeAvatar = async (req, res, next) => {
    ...
    await image.resize(250, jimp.AUTO); // resize image
    await image.write(`./${req.body.avatar}`); // include relative path to add to static folder
    next();
};
...
exports.updateUser = async (req, res) => {
    ...
    const updatedUser = await User.findOneAndUpdate(
        { _id: req.user._id },
        { $set: req.body},
        { new: true, runValidators: true} // re run validation 
    )
